...
class CLIPLaBo(nn.Module):
    def __init__(self, list_concepts, list_classes, device):
        super().__init__()
        """
		Initializes a CLIP_QDA instance.
        ...
        # LaBo parameters
        self.asso_mat = nn.Parameter(
            torch.randn(len(list_classes), len(list_concepts)), requires_grad=True
        )
    ...
    def _get_weight_mat(self):
        ...
        """
        return F.softmax(self.asso_mat, dim=-1).to(self.device)
    ...
    def test(self, data_in, data_out):
        """Computes the accuracy score of the trained labo model on the provided data.
    ...
    def infer_from_images(self, x):
        ...
        Parameters:
        x (PIL.Image.Image or np.ndarray): Input image.
        ...
        return torch.argmax(similarity @ mat.t())
    ...
    def infer_proba_from_scores(self, input_score):
        ...
        return F.softmax(input_score @ mat.t())
    ...
    def plot_labo_explanation(
        ...
        """Plots the labo explanation for the input image_scores.
        ...
        a_heatmap_classes = self._get_weight_mat().cpu().detach().numpy()  # N_classes , N_concepts
